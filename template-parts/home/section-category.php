<section id="nsc-home-category" class="nsc-home-category">
    ...
    <?php foreach ($categories as $key => $category) { ?>
      <li class="nav-item" role="presentation">
        <button
          ...
          id="<?php echo strtolower(sanitize_title($category->name)); ?>-tab"
          data-bs-toggle="tab"
          data-bs-target="#<?php echo strtolower(sanitize_title($category->name)); ?>"
          type="button"
          role="tab"
          aria-controls="<?php echo strtolower(sanitize_title($category->name)); ?>"
          aria-selected="<?php echo $key == 0 ? 'true' : 'false'; ?>">
          <a href="#<?php echo strtolower(sanitize_title($category->name)); ?>">
  ...
  <div class="tab-content" id="nsc-home-tab-containerContent">
    <?php foreach ($categories as $key => $category) {
      ...
      if ($query->have_posts()) { ?>
        <div class="tab-pane fade <?php echo $key == 0 ? 'show active' : ''; ?> nsc-blog-post-grid"
             id="<?php echo strtolower(sanitize_title($category->name)); ?>"
             role="tabpanel"
             aria-labelledby="<?php echo strtolower(sanitize_title($category->name)); ?>-tab"
             tabindex="0">
                ...
                <div class="d-flex align-items-center gap-2">
                  <div class="d-flex align-items-center gap-2">
                    ...
                    if (!empty($avatar_html)) {
                      $dom = new DOMDocument;
                      @$dom->loadHTML($avatar_html); // Suppress warnings from malformed HTML
                    ...
                    if (get_theme_mod('nsc_blog_single_post_author_name', true) != '0') { ?>
